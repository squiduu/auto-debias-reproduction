...
def p_val_permutation_test(X, Y, A, B, n_samples, cossims, parametric: bool, logger: Logger):
    ...
    if parametric:
        ...
        logger.info("Draw {} samples.".format(n_samples))
        samples = []
...
def run_test(encs: Dict[str, np.ndarray], num_samples: int, use_parametric: bool, logger: Logger):
    ...
    pval = p_val_permutation_test(X, Y, A, B, num_samples, cossims=cossims, parametric=use_parametric, logger=logger)
    logger.info("p-val: %g", pval)
